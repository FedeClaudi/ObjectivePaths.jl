...
function Base.print(io::IO, p::AbstractPath)
    path = RenderableText("{bright_blue}$(title(p)){/bright_blue} {italic}" * highlight_path(p.path) * "{italic}"; style="bold white")
...
function info(f::Folder)
    path = RenderableText(title(f) * highlight_path(f.path) * "{italic}"; style="bold white")
    ...
    if !exists(f)
        ...
        values = [
            exists(f) ? "{green}yes{/green}" : "{red}no{/red}",
            nfiles(f)
        ]

        tb = Panel(Table(
            OrderedDict(
                ""=>properties, :value=>string.(values)
            ),
